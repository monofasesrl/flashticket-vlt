/*
  # Fix demo tickets function

  1. Changes
    - Drop existing function
    - Create new function with proper schema qualification and permissions
    - Add proper error handling
    - Add return value to indicate success/failure
*/

-- Drop existing function if exists
DROP FUNCTION IF EXISTS public.insert_demo_tickets();

-- Create function to insert demo tickets
CREATE OR REPLACE FUNCTION public.insert_demo_tickets()
RETURNS boolean
SECURITY DEFINER
...
BEGIN
  ...
  IF v_user_id IS NULL THEN
    RAISE EXCEPTION 'User must be authenticated to insert demo tickets';
  END IF;

  -- Insert demo tickets within a transaction
  BEGIN
    -- MacBook Pro repair
  ...
  EXCEPTION
    WHEN OTHERS THEN
      RAISE NOTICE 'Error inserting demo tickets: %', SQLERRM;
      RETURN false;
  END;
END;
$$;
...
REVOKE ALL ON FUNCTION public.insert_demo_tickets() FROM public;

-- Grant execute to authenticated users
GRANT EXECUTE ON FUNCTION public.insert_demo_tickets() TO authenticated;

-- Set proper search path
ALTER FUNCTION public.insert_demo_tickets() SET search_path = public, auth;
